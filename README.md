...
| formTab  <br>`<form-tab tab-title="Validation">` | E      | Allow you to add tabs to a form in combination with form-tabs as a parent element. If you have more than one tab you can navigate to the next tab only when all form controls in the current tab are valid. <br>Attribute `tab-title"` sets the tab title label. <br>Attribute `next-tab-button-label` sets next button label.|
