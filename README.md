...
| errorMessage <br>`<error-message>`        | E      | Use it inside `<form-control>` to display error messages. You must add a class with the same name of angular validation directive. `<error-message class="ng-required">`. Error message show only when input is ng-dirty ng-invalid. |
